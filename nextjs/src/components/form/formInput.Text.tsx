...
const FormInputText: React.FC<Props> = ({ label, className, prefix, postfix, register, errors, ...inputprops }) => {
  return (
      ...
      <div className="mt-2 relative">
        <div className={cn(errors[register.name] && "border border-red-600", "flex rounded-md shadow-sm ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 sm:max-w-md")}>
          ...
          <input
            ...
            {...inputprops}
          />
          {postfix && (<span className="flex select-none items-center pr-3 text-gray-500 sm:text-sm">{postfix}</span>)}
        </div>

...
export const removeDomainFromVercelProject = async (domain: string) => {
  ...
  ).then((res) => res.json() as Promise<RemoveDomainResponse>);
};
...
export const removeDomainFromVercelTeam = async (domain: string) => {
  ...
  ).then((res) => res.json() as Promise<RemoveDomainByNameResponse>);
};
...
export const getDomainResponse = async (
  ...
  return await fetch(
    ...
    return res.json() as Promise<DomainResponse & { error: { code: string; message: string } }>;
  });
};
